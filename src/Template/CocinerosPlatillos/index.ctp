...
<div class="cocinerosPlatillos index large-9 medium-8 columns content">
    <h3><?= __('Cocineros Platillos') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th scope="col"><?= $this->Paginator->sort('id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('cocinero_id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('platillo_id') ?></th>
                <th scope="col" class="actions"><?= __('Actions') ?></th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($cocinerosPlatillos as $cocinerosPlatillo): ?>
            <tr>
                ...
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $cocinerosPlatillo->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $cocinerosPlatillo->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $cocinerosPlatillo->id], ['confirm' => __('Are you sure you want to delete # {0}?', $cocinerosPlatillo->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
